...
def invite_to_event(current_user, event_id):
    ...
    try:
        ...
        event = db.execute(
            'SELECT username, name, is_private FROM events WHERE id = ?',
            (event_id,)
        ).fetchone()
        ...
        for username in usernames:
            try:
                ...
                # Cria o convite
                db.execute(
                    'INSERT INTO event_invites (event_id, invited_username, invited_by) VALUES (?, ?, ?)',
                    (event_id, username, current_user['username'])
                )
                
                # Notifica o usuário
                db.execute(
                    'INSERT INTO notifications (username, type, content) VALUES (?, ?, ?)',
                    (username, 'event_invite', f"Convite para evento: {event['name']}")
                )
...
def get_my_event_invites(current_user):
    ...
    try:
        ...
        invites = db.execute(
            '''SELECT ei.id, ei.event_id, ei.invited_by, ei.created_at, ei.status,
                      e.name, e.description, e.start_time, e.latitude, e.longitude
...
def respond_to_invite(current_user, invite_id):
    ...
    try:
        ...
        invite = db.execute(
            '''SELECT ei.id, ei.event_id, ei.status, e.name, e.max_participants,
        ...
        log_action("EVENT_INVITE_RESPONDED", current_user['username'], 
                  f"Resposta: {response} para evento: {invite['name']}")
        
        return jsonify({
            "status": "success",
            "message": f"Convite {response} com sucesso",
            "event_name": invite['name']
        })
